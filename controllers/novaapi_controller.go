...
// GetClient -
func (r *NovaAPIReconciler) GetClient() client.Client {
	return r.Client
}

// GetLogger -
func (r *NovaAPIReconciler) GetLogger() logr.Logger {
	return r.Log
}

// GetScheme -
func (r *NovaAPIReconciler) GetScheme() *runtime.Scheme {
	return r.Scheme
}
...
func (r *NovaAPIReconciler) deploymentCreateOrUpdate(instance *novav1beta1.NovaAPI, envVars map[string]util.EnvSetter) (controllerutil.OperationResult, error) {
	runAsUser := int64(0)

	// set KOLLA_CONFIG env vars
	envVars["KOLLA_CONFIG_FILE"] = util.EnvValue(novaapi.KollaConfig)
	...
	op, err := controllerutil.CreateOrUpdate(context.TODO(), r.Client, deployment, func() error {
		...
		if deployment.ObjectMeta.CreationTimestamp.IsZero() {
			deployment.Spec.Selector = &metav1.LabelSelector{
				MatchLabels: common.GetLabels(instance.Name, novaapi.AppLabel),
			}
		}
		...
		for k, v := range common.GetLabels(instance.Name, novaapi.AppLabel) {
		...
		initContainerDetails := common.CtrlInitContainer{
			...
			CellDatabase:       fmt.Sprintf("%s_%s", novaapi.DatabasePrefix, novaapi.CellDatabase),
			APIDatabase:        fmt.Sprintf("%s_%s", novaapi.DatabasePrefix, novaapi.APIDatabase),
