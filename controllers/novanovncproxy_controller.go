...
// GetClient -
func (r *NovaNoVNCProxyReconciler) GetClient() client.Client {
	return r.Client
}

// GetLogger -
func (r *NovaNoVNCProxyReconciler) GetLogger() logr.Logger {
	return r.Log
}

// GetScheme -
func (r *NovaNoVNCProxyReconciler) GetScheme() *runtime.Scheme {
	return r.Scheme
}
...
func (r *NovaNoVNCProxyReconciler) deploymentCreateOrUpdate(instance *novav1beta1.NovaNoVNCProxy, envVars map[string]util.EnvSetter) (controllerutil.OperationResult, error) {
	runAsUser := int64(0)

	// set KOLLA_CONFIG env vars
	envVars["KOLLA_CONFIG_FILE"] = util.EnvValue(novanovncproxy.KollaConfig)
	...
	op, err := controllerutil.CreateOrUpdate(context.TODO(), r.Client, deployment, func() error {
		...
		if deployment.ObjectMeta.CreationTimestamp.IsZero() {
			deployment.Spec.Selector = &metav1.LabelSelector{
				MatchLabels: common.GetLabels(instance.Name, novanovncproxy.AppLabel),
			}
		}
		...
		for k, v := range common.GetLabels(instance.Name, novanovncproxy.AppLabel) {
		...
		initContainerDetails := common.CtrlInitContainer{
			...
			CellDatabase:       fmt.Sprintf("%s_%s", novanovncproxy.DatabasePrefix, instance.Spec.Cell),
			APIDatabase:        fmt.Sprintf("%s_%s", novanovncproxy.DatabasePrefix, novanovncproxy.APIDatabase),
