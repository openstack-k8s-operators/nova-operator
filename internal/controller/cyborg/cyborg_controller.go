...
func (r *CyborgReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	//
	_, result, inputSecret, err := internalcommon.EnsureSecret(
		ctx,
	...
	// TransportURL Secret
	_, result, transporturlSecret, err := internalcommon.EnsureSecret(
		ctx,
...
func (r *CyborgReconciler) reconcileDelete(
	...
	// finalizer from ourselves to allow the deletion of Cyborg CR itself
...
func (r *CyborgReconciler) findObjectsForSrc(ctx context.Context, src client.Object) []reconcile.Request {
	return internalcommon.FindObjectsForSrcByField(
		ctx, r.GetLogger(ctx), r.Client, src, cyborgWatchFields,
		func() client.ObjectList { return &cyborgv1beta1.CyborgList{} },
	)
}
