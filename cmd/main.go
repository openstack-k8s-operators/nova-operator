...
import (
	...
	"github.com/openstack-k8s-operators/nova-operator/internal/controller"
...
func main() {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupNovaAPIWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaAPI")
			os.Exit(1)
		}

		if err := webhookv1beta1.SetupNovaSchedulerWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaScheduler")
			os.Exit(1)
		}

		if err := webhookv1beta1.SetupNovaConductorWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaConductor")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupNovaMetadataWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaMetadata")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupNovaNoVNCProxyWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaNoVNCProxy")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupNovaCellWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "NovaCell")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupNovaWebhookWithManager(mgr); err != nil {
