...
var _ = Describe("NovaCell controller", func() {
	...
	When("A NovaCell/cell1 CR instance is created", func() {
		...
		It("does not advance AppliedInputSecretHash until all cell services roll out the rotated input", func() {
			...
			// The compute still reports the previous rollout, so it must no
			// longer count as deployed for the current cell input.
			Eventually(func(g Gomega) {
				cell := GetNovaCell(cell1.CellCRName)
				g.Expect(cell.Status.NovaComputesStatus).To(HaveKey(ironicComputeName))
				g.Expect(cell.Status.NovaComputesStatus[ironicComputeName].Deployed).To(BeFalse())
				g.Expect(cell.Status.AppliedInputSecretHash).To(Equal(oldHash))
			}, timeout, interval).Should(Succeed())
