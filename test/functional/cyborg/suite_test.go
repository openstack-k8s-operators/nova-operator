...
import (
	...
	controllers "github.com/openstack-k8s-operators/nova-operator/internal/controller/cyborg"
...
var _ = BeforeSuite(func() {
	...
	err = cyborgv1beta1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	cyborgv1beta1.SetupDefaults()

	err = cyborgwebhookv1beta1.SetupCyborgWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
	err = cyborgwebhookv1beta1.SetupCyborgAPIWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
	err = cyborgwebhookv1beta1.SetupCyborgConductorWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
...
var _ = BeforeEach(func() {
	...
	cyborgName := types.NamespacedName{
		Namespace: namespace,
		Name:      "cyborg-" + uuid.New().String()[:10],
	}
	cyborgNames = GetCyborgNames(cyborgName)
})
